...
#        out = num if int(num) > 0 else format(num, '.0e')
    ...
    def set_filter_window(self, window: float,
        ...
        enable = meas.filter_on
        inf = info.fwindow
        if not inf['lim'][0] <= window <= inf['lim'][1]:
            window = inf['def']
            print(f"Filter window out of bounds ([{inf['lim'][0]}, "
                  + f"{inf['lim'][1]}]).  Setting to default value "
                  + f"({window}).")
    ...
    def set_filter_count(self, count: int,
        ...
        enable = meas.filter_on
        inf = info.fcount
        if count not in inf['lim']:
            count = inf['def']
            print(f"Filter count out of bounds ([{inf['lim'][0]}, "
                  + f"{inf['lim'][1]}]).  Setting to default value ({count}).")
